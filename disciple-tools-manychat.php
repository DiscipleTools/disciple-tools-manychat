...
function dt_manychat() {
     ...
     */
    $is_rest = dt_is_rest();
    if ( ! $is_rest || strpos( dt_get_url_path(), 'dt-public' ) != false ){
        return DT_Manychat::get_instance();
    }
}
add_action( 'after_setup_theme', 'dt_manychat' );
...
class DT_Manychat {
    ...
    private function setup() {
        ...
        DT_Manychat_Endpoints::instance();
    }
    ...
    public static function deactivation() {
        delete_option( 'dismissed-dt-manychat' );
    }
    ...
    public function i18n() {
        load_plugin_textdomain( 'dt_manychat', false, trailingslashit( dirname( plugin_basename( __FILE__ ) ) ). 'languages' );
    }
    ...
    public function __clone() {
        _doing_it_wrong( __FUNCTION__, esc_html__( 'Whoah, partner!', 'dt_manychat' ), '0.1' );
    }
    ...
    public function __call( $method = '', $args = array() ) {
        ...
        _doing_it_wrong( "dt_manychat::{$method}", esc_html__( 'Method does not exist.', 'dt_manychat' ), '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
function dt_manychat_hook_admin_notice() {
    ...
    if ( ! get_option( 'dismissed-dt-manychat', false ) ) { ?>
        <div class="notice notice-error notice-dt-manychat is-dismissible" data-notice="dt-manychat">
            <p><?php echo esc_html( $message );?></p>
        </div>
        <script>
            jQuery(function($) {
                $( document ).on( 'click', '.notice-dt-manychat .notice-dismiss', function () {
                    $.ajax( ajaxurl, {
                        type: 'POST',
                        data: {
